...
async def DeleteRemoteAgent(path: str) -> bool:
    client = ConversationClient(server_url)
    try:
        response = await client.unregister_agent(UnregisterAgentRequest(params=path))
        return response.result if response.result is not None else False
    except Exception as e:
        print('Failed to unregister the agent', e)
    return False
...
def convert_task_to_state(task: Task) -> StateTask:
    ...
    output = (
        [extract_content(a.artifacts) for a in task.artifacts]
        if task.artifacts
        else []
    )
    if not task.history:
