...
def agents_list(
    ...
    def on_agent_row_click(event: me.ClickEvent):
        state = me.state(AgentState)
        # Assuming event.value contains the row data as a dictionary
        agent_address = event.value['Address']
        state.delete_agent(agent_address)
