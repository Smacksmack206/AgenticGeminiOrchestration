...
from samples.python.hosts.multiagent.remote_agent_connection import RemoteAgentConnections, TaskUpdateCallback
from samples.python.hosts.multiagent.timestamp_ext import TimestampExtension
...
class HostAgent:
    ...
    def __init__(
        ...
        # Don't auto-initialize in constructor to allow UI server to control timing

    async def initialize_remote_agents(self):
        """Initialize remote agents - called by UI server."""
        await self.init_remote_agent_addresses(self._remote_agent_addresses)
    ...
    def root_instruction(self, context: ReadonlyContext) -> str:
        ...
        return f"""You are an expert delegator that routes user requests to appropriate remote agents.
...
- When you receive a response from a remote agent via `send_message`, you MUST format your response as follows:
  **[Agent Name]**: [Exact agent response without any modification]
- DO NOT summarize, paraphrase, or modify the agent's response
- DO NOT add commentary like "The agent provided..." or "Okay, the agent said..."
- The format should be: **Coder Agent**: [response] or **VEO Video Generation Agent**: [response]
- This ensures the user knows which specific agent provided the response

Example:
User: "Write hello world in Python"
You call send_message to Coder Agent
Agent responds with code
Your response: **Coder Agent**: ```python\nprint("Hello, World!")\n```

If you are not sure about something, ask the user for more details.
...
"""
    ...
    async def send_message(
        ...
        if task.status.state == TaskState.input_required:
            ...
            # Don't raise error, return the agent's error message instead
            pass  # Continue to process the response normally
        response = []
        ...
        # Add visual enhancement for agent responses
        if response:
            # Add agent name with appropriate emoji indicator
            if "VEO" in agent_name or "Video" in agent_name:
            ...
            if isinstance(response[0], str):
                response[0] = agent_prefix + response[0]
            else:
                response.insert(0, agent_prefix)
